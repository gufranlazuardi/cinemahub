...
import { getUpcomingList, MovieItem } from "@/utils/apis/index";

import { CornerUpLeft } from "lucide-react";

const ListUpcoming = () => {
  const [dataUpcomings, setDataUpcomings] = useState<MovieItem[]>([]);
  ...
  async function fetchData() {
    try {
      ...
      setDataUpcomings(result.results);
    } catch (error: any) {
      toast({
    ...
    <Layout>
      <div className="flex flex-col">
        <div className="flex justify-around text-center items-center py-8">
          ...
          <h1 className="text-4xl font-bold py-2 mx-auto">Upcoming</h1>
        </div>
        ...
        <div className="py-5 gap-6 grid grid-cols-6">
          {dataUpcomings.map((upcoming, index) => (
            <MovieCardOnList data={upcoming} key={index} />
          ))}
        </div>
      </div>
    </Layout>
